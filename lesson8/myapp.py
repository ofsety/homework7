...
class Point():
    ...
    def falls_in_rectangle(self, point1_coordinate, point2_coordinate):
        ...
        if self.point1_coordinate.x <= self.x <= self.point2_coordinate.x and self.point1_coordinate.y <= self.y <= self.point2_coordinate.y:
            global win_counter_global
            win_counter_global +=1
            return True
            

        elif self.point1_coordinate.x >= self.x >= self.point2_coordinate.x and self.point1_coordinate.y >= self.y >= self.point2_coordinate.y:
            win_counter_global +=1
            return True
...
while termination != "Yes":
    ...
    if user_point.falls_in_rectangle == True:
        print(f"""
                The point falls in rectangle
                Your win counter is {win_counter_global}
                {point1},{point2}
     """)
